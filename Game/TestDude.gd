...
func old(delta: float) -> void:
    var preLayer := layerFromZ(currentZ)
    var preMapCoords := mapPositionFromScreenPosition(global_position, currentZ)

    # Add the gravity.
    var distFromGround = currentZ - preMapCoords.groundHeight
    var currentlyOnGround = (preMapCoords.groundType != GroundType.EMPTY) and (distFromGround < (layerHeight / 4))
    #if not currentlyOnGround:
    #    velocityZ -= gravity * delta
    ...
    if deltaZ < -layerHeight:
        deltaZ = -layerHeight
    var postLayer = layerFromZ(currentZ + deltaZ)

    # Don't allow to fall through the solid ground.
    # We don't check it very correctly, because we use final position, but pre and post layer.
    var postMapCoordsOldZ = mapPositionFromScreenPosition(global_position, currentZ)
    #if (postLayer < preLayer) and (postMapCoordsOldZ.groundType != GroundType.EMPTY):
    #    deltaZ = max(deltaZ, postMapCoordsOldZ.groundHeight - currentZ)

    # Update postLayer and check for slopes.
    # We check all 3 layers around us, assuming that if any of those is a slope we are on it...
    postLayer = layerFromZ(currentZ + deltaZ)
    var slopeAbove = slopeGroundHeight(postMapCoordsOldZ.mapCoords, postMapCoordsOldZ.planarOffsetWithinTile, postLayer + 1)
    var slopeHere = slopeGroundHeight(postMapCoordsOldZ.mapCoords, postMapCoordsOldZ.planarOffsetWithinTile, postLayer)
    var slopeBelow = slopeGroundHeight(postMapCoordsOldZ.mapCoords, postMapCoordsOldZ.planarOffsetWithinTile, postLayer - 1)
    var onSlope := false
    var slopeHeight := 0.0
    if slopeAbove != null:
        onSlope = true
        slopeHeight = slopeAbove
    if slopeHere != null:
        onSlope = true
        slopeHeight = slopeHere
    if slopeBelow != null:
        onSlope = true
        slopeHeight = slopeBelow

    # If we stepped onto a slope, glue player to the slope.
    # We can be on a slope down (so on our layer) or on slope up (on layer above).
    # We will never have slope on both.
    if currentlyOnGround and onSlope:
        #deltaZ = slopeHeight - currentZ
        pass

    if onSlope:
        deltaZ = slopeHeight - currentZ
    
    # Move in Z, and correct the 2D position to account for the projection.
    currentZ += deltaZ
    position.y -= deltaZ * zToYOffsetRatio
    
    var finalLayer = layerFromZ(currentZ)

    # Update collisions. When on slopes we don't have any layer collisions.
    clearLayerCollisionMask()
    var collisionsLayer = null
    #if not onSlope:
    #    setLayerCollisionMask(finalLayer, true)
    #    collisionsLayer = finalLayer

    debugLabel.text = "onSlope={onSlope} onGround={onGround} mapCoords={mapCoords} currentZ={currentZ} collisionsLayer={collisionsLayer}\noffsetWithinTile={offsetWithinTile} planarOffsetWithinTile={planarOffsetWithinTile} slopeHeight={slopeHeight}".\
        format({"onSlope": onSlope, "onGround": currentlyOnGround, "mapCoords": postMapCoordsOldZ.mapCoords, "currentZ": currentZ, "collisionsLayer": collisionsLayer, "offsetWithinTile": postMapCoordsOldZ.offsetWithinTile, "planarOffsetWithinTile": postMapCoordsOldZ.planarOffsetWithinTile, "slopeHeight": slopeHeight})
